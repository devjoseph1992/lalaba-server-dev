...
export const verifyFirebaseToken = async (
  ...
  try {
    ...
    console.log(
      `✅ Verified Token for UID: ${decodedToken.uid}, Role: ${req.user.role}`
    );
...
export const isAdmin = async (
  ...
  try {
    ...
    if (!req.user || req.user.role !== "admin") {
      console.warn(
        "🚫 Access Denied: Non-admin user attempted to access an admin-only route."
      );
    ...
    return res
      .status(500)
      .json({ message: "Error checking admin role.", error });
  }
};
...
export const isAdminOrEmployee = async (
  ...
  try {
    if (
      ...
      return res
        .status(403)
        .json({ message: "Admin or Employee access required." });
    }
    return next();
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ message: "Error checking admin/employee role.", error });
  }
};
...
export const isEmployee = async (
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error checking employee role.", error });
  }
};
...
export const isRider = async (
  ...
  try {
    ...
    if (!req.user || req.user.role !== "rider") {
      console.warn(
        "🚫 Access Denied: Non-rider user attempted to access a rider-only route."
      );
    ...
    return res
      .status(500)
      .json({ message: "Error checking rider role.", error });
  }
};
...
export const isMerchant = async (
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error checking merchant role.", error });
  }
};
